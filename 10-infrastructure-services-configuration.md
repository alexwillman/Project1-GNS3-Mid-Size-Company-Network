...
| NTP not syncing on L2 switches only | Likely that the L2 switches are missing the `no ip routing` command. This causes the switch to ignore the default gateway set and are then not able to return traffic. On each L2 switch having the problem, run `enable` then `configure terminal` then `no ip routing`. Make sure to save with `do write`. |
| pfSense NTP shows Unreach/Pending | Add `allow 10.0.0.0/24` to the chrony config file using the command `sudo nano /etc/chrony/chrony.conf`. |
| apache2 fails to start | Verify no other service is using port 80 with `sudo ss -tlnp`. If another service is using port 80 then run `sudo systemctl stop [service name]` then restart apache2 with `sudo systemctl restart apache2`. |
