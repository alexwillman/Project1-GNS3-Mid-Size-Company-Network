...
servers via access ports with VLAN segmentation, and an end device/server layer with department end devices and Ubuntu servers handling network services.
...
The edge consists of a pfSense firewall which provides firewall rules throughout the network and routes traffic to the simlulated ISP. The WAN interface connects to the simulated ISP and the LAN interfaces connect to both core switches, providing redundancy. (Note: For true redundancy and high availability,
...
This tier consists of three layer 2 switches which hold the VLANs for the network, connect the end devices to the network, and ensure there are no switching loops using Rapid PVST+. The access switches are connected to the ethernet ports on each end device and server, and segment the departments through VLANs.
...
| pfSense Firewall | pfSense-Firewall | pfSense 2.7.2 | Edge | Provide security using firewall rules and connects the internal network to the ISP on its WAN interface |
| Layer 3 Switch | L3-Multilayer-SW1 | IOSv-L2 | Core/Distribution | Route traffic through the network and provide high availability using HSRP active on VLANs 10, 20, 30 |
| Layer 3 Switch | L3-Multilayer-SW2 | IOSv-L2 | Core/Distribution | Route traffic through the network and provide high availability using HSRP active on VLANs 40, 50, 60 |
...
| Layer 2 Switch | L2-SW2 | IOSv-L2 | Access | Provides layer 2 switching with access ports to the Finance and IT departments and trunk links to both core switches |
